...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    for c in 0..columns {
        let numbers =
            (0..=number_rows).map(|r| get_value(&values, columns, r, c).parse::<u64>().unwrap());
        let operator = get_value(&values, columns, rows - 1, c);
        let value: u64 = if operator == "+" {
            numbers.sum()
        } else if operator == "*" {
            numbers.product()
        } else {
...
pub fn get_value<'a>(values: &'a [&str], width: usize, row: usize, col: usize) -> &'a str {
...
pub fn index(width: usize, row: usize, col: usize) -> usize {
    row * (width + 1) + col
}
