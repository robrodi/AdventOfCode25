...
fn validate_number_1(number: u64) -> bool {
    ...
    if debug {
        print!("Number: {}. num_digits: {}. ", number, num_digits);
    }
    if !num_digits.is_multiple_of(2) {
        if debug {
            println!("Odd number of digits, automatically valid.");
        }
        return true;
    }
    ...
    let result = l != r;
    if debug {
        println!("split = {}. L: {}, R: {}. Result: {}", split, l, r, result);
    } //if result { "✓".green().bold() } else { "x".red().bold() });
...
fn validate_number_2(number: u64) -> bool {
    ...
    if debug {
        println!(
            "    Number: {}, Max repeat segment length: {}.",
            number, max_repeat
        );
    }
    let s = number.to_string();
    ...
    for i in 1..=max_repeat {
        if !(s.len() as u32).is_multiple_of(i) {
            continue;
        }
        let seg = &s[0..i as usize];
        if debug {
            println!("    Checking segment [{}] length: {}", seg, i);
        }
        ...
        for j in 1..(s.len() as u32 / i) {
            ...
            if next_seg != seg {
                if debug {
                    println!(
                        "    Found differing segment [{}] at position {}. No Repition of Size {} with sample {}",
                        next_seg, j, i, seg
                    );
                }
                repetition = false;
        ...
        if repetition {
            if debug {
                println!(
                    "    {}: Number {} Repition of Size {} with sample {}",
                    "x".red().bold(),
                    number,
                    i,
                    seg
    ...
    if debug {
        println!("  {}: Number {} ", "✓".green().bold(), number);
    }
    true
}
...
mod tests {
    ...
    fn simple_part_two_number_validation() {
        let good = [1, 12, 12321];
        let bad = [
            11, 111, 11111111, 22222222, 565656, 1010, 1188511885, 824824824,
        ];
    ...
    fn simple_part_two_range_validation() {
        ...
        assert_eq!(
            validate_range_2(824824821, 824824827).collect::<Vec<u64>>(),
            vec![824824824]
        );
    }
